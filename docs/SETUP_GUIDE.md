# Setup Guide - Soup Cookoff Digital Voice Assistant
...
3. Name it (e.g., "Soup Cookoff Voice Assistant")
...
```sql
CREATE DATABASE soupcookoff;
CREATE USER soupcookoff WITH PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE soupcookoff TO soupcookoff;
```
...
```env
DATABASE_URL=postgresql://user:password@localhost:5432/soupcookoff
...
This creates:
- Sample events (Carlisle, Harrisburg)
...
- [ ] Ask "When is the next event?"
- [ ] Ask "How much are tickets?"
- [ ] Test ticket purchase flow (use test card)
...
DATABASE_URL=postgresql://user:pass@prod-host:5432/soupcookoff
...
**Azure App Service:**
```bash
az webapp up --name soup-cookoff --resource-group your-rg
```
