...
This document outlines a proposal to enhance the SCO Digital Assistant by offloading complex business logic and integrations to **n8n** (an AI-native workflow automation platform).
...
graph LR
    ...
    Twilio -- Stream --> Server[SCO Assistant Server]
...
### 1. Smart Sponsor Inquiry Handling

**Problem**: Currently, `captureSponsorInquiry` just logs to DB and sends a generic Slack/SMS.
...
4.  **Logical Branching**:
    *   *If High Value (>$1000)*: SMS the Event Director immediately + Create High Priority Task.
    *   *If Standard*: Send welcome email + Add to marketing sequence.
5.  **Slack Notification**: Post formatted card to `#sponsorships`.
...
2.  **Scrape**: Fetch content from `soupcookoff.com/faq`, `/rules`, `/events`.
3.  **LLM Extract**: Use an LLM node to extract Q&A pairs and format them as Markdown.
4.  **API Call**: Call the SCO Assistant's Admin API (or write to DB directly) to update the Knowledge Base documents.
...
4.  **Action**:
    ...
    *   *If "Buying Ticket" failed*: Send retargeting SMS with a discount code.
...
example modification for `captureSponsorInquiry`:
...
// ... inside captureSponsorInquiry ...
// Fire and forget (don't block the voice response)
triggerWorkflow('sponsor-inquiry', {
...
return {
  ok: true,
  message: "I've passed your info to our sponsorship team..."
};
```
