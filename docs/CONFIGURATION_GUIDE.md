...
This guide explains how to use the Configuration features in the Soup Cookoff Voice Assistant Admin Panel.
...
| `ticket_purchased` | Ticket sale completed |
| `sponsor_inquiry` | Sponsor interest captured |
...
- **Ticket Purchase Confirmation** - Send confirmation with ticket details
- **Sponsor Follow-up** - Thank sponsors for their interest
- **Event Reminders** - 24-hour reminder before events
...
|------------|--------------|-------------|
| `sales` | +15551234567 | Sales team line |
| `support` | +15551234568 | Customer support |
| `sponsorship` | +15551234569 | Sponsorship inquiries |
...
|-------|-------|--------|-------|
| 1 | Event Info | `say` | "The next event is..." |
| 2 | Buy Tickets | `ai_intent` | purchaseTickets |
| 3 | Sponsorship | `ai_intent` | getSponsorshipInfo |
...
| `ai_intent` | Trigger AI function | purchaseTickets |
| `repeat` | Replay menu | - |

### Example Menu Greeting

```
Welcome to the Soup Cook Off.
Press 1 for event information.
Press 2 to purchase tickets.
Press 3 for sponsorship opportunities.
...
**Information Tools:**
- `getEventInfo` - Retrieve upcoming event details
- `getTicketPricing` - Get ticket prices
- `getSponsorshipInfo` - Sponsorship package info
- `getPastWinners` - Competition history
- `getChefInfo` - Chef registration info
- `getAboutInfo` - Organization information
- `answerQuestion` - Knowledge base queries

**Payment Tools:**
- `purchaseTickets` - Process ticket purchases
...
- `captureSponsorInquiry` - Record sponsor interest
...
**Main English Agent:**
```
You are a helpful assistant for The Soup Cook Off charity event.
...
**Spanish Agent:**
```
Eres un asistente útil para The Soup Cook Off.
Habla siempre en español. Ayuda a los llamantes con:
- Información del evento
- Compra de boletos
- Consultas de patrocinio
...
**Sales-Focused Agent:**
```
You are a sales-focused assistant for The Soup Cook Off.
Your primary goal is to help callers purchase tickets.
Highlight the benefits: great food, charity cause, entertainment.
Mention VIP upgrades when appropriate.
...
**5. Ticket Intent Notification (Priority: 5)**
```
Condition: intent == 'purchase_tickets'
Action: webhook
Value: ticket_interest
```
